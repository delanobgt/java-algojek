...
public class AlgoSendOrderScreen {
    ...
    private void orderMethod() { //finding customer method
        String loadingSprite = Tool.getStringFromTextFile("res\\algojobs\\send.txt", 25);
        ...
        while (true) {
            ...
            int itemIdx = (int)(Math.random()*itemList.size());
            String item = itemList.get(itemIdx);
            ...
            for (int i = 8; i >= 1; i--) {
                ...
                customerDetails = String.format("\n%s%-14s | %-10s | %-11s | %-6s | %-4s | %s \n", Tool.rep(' ',2),
                                        "Customer Name", "Item Name", "Job Fare", "Energy", "Fuel", "Trip Path") +
                                Tool.rep(' ',2)+Tool.rep('-', 95) + "\n" +
                                String.format("%s%-14s | %-10s | +Rp. %,-6d | %6s | %4s | %s\n", Tool.rep(' ',2),
                                        name+" ("+gender+")", item, money, "-"+energy, "-"+fuel+"%",
